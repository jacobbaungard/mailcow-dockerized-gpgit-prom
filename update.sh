...
while (($#)); do
  case "${1}" in
    --check|-c)
      ...
      LATEST_REV=$(git ls-remote --exit-code --refs --quiet https://github.com/jacobbaungard/mailcow-dockerized-gpgit-prom ${BRANCH} | cut -f1)
      if [ "$?" -ne 0 ]; then
...
DEFAULT_REPO=https://github.com/jacobbaungard/mailcow-dockerized-gpgit-prom
